    ...
    <div class="container-page">
      <div class="row article-content">
        <div class="col-xs-12">
          ...
          <ul class="tag-list">
            <li v-for="(tag, index) of article.tagList"
                :key="tag + index">
              <RwvTag :name="tag"
                      className="tag-default tag-pill tag-outline"></RwvTag>
            </li>
          </ul>
        </div>
      </div>
      ...
      <div class="row">
        <div class="col-xs-12 col-md-8 offset-md-2">
          <RwvCommentEditor v-if="isAuthenticated"
                            :slug="slug"
                            :userImage="currentUser.image"></RwvCommentEditor>
          <p v-else>
            <router-link
                :to="{name: 'login'}"
            >Sign in
